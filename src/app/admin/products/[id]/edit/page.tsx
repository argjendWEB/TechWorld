...
export default function EditProduct({ params }: { params: { id: string } }) {
        ...
        fetch(`/api/admin/products?id=${id}`).then(r => r.json()).then(data => {
            if (data.product) {
                ...
                setForm({
                    ...
                    image_url: p.image_url || '', category: p.category, badge: p.badge || '',
                    stock: String(p.stock), is_active: p.is_active,
                    features: p.features?.length ? p.features : ['', '', '', ''],
                    gallery_images: p.gallery_images?.length ? p.gallery_images : [''],
                });
            }
            setLoading(false);
        });
    ...
    function updateField(field: string, value: string | boolean) {
        setForm(prev => {
            ...
            if (field === 'title' && !slugEdited) next.slug = slugify(value as string);
            return next;
        });
    }
    ...
    function updateGallery(i: number, val: string) {
        setForm(prev => { const gallery_images = [...prev.gallery_images]; gallery_images[i] = val; return { ...prev, gallery_images }; });
    }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        const body = {
            ...
            badge: form.badge, stock: parseInt(form.stock) || 0,
    ...
    if (loading) return <div className="flex justify-center py-20"><div className="animate-spin w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full" /></div>;
